...
if __name__ == '__main__':
    ...
    if model_type == 'url':
        ...
        # Domain: Use real WHOIS data for validation
        # If domain has known registrar and is old, it's valid
        domain_age = details.get('domain_age_days', 0)
        is_known = details.get('is_known_registrar', False)
        is_new = details.get('is_new_domain', False)
        is_high_risk = details.get('is_high_risk_country', False)

        if is_known and not is_new and not is_high_risk and domain_age > 365:
            result_str = 'valid'
            confidence = 90.0
        elif is_new or is_high_risk:
